...
int main (int argc, char * argv[]) {
    ...
    fprintf(stderr, "running with %d procs and %d threads\n", num_procs, omp_get_max_threads() );
    ...
    // each thread do its alignment
    #pragma omp parallel private(omp_rank)
    {
	omp_rank = omp_get_thread_num();
	char line1 [MAXLINELEN];
	char line2 [MAXLINELEN];
	int line_num = (rank * num_threads) + omp_rank;
	strcpy(line1, lineAs[line_num]);
	strcpy(line2, lineBs[line_num]);                
	
	// fprintf(stderr, "process %d, thread %d, seq %s and %s\n", rank, omp_rank, line1, line2);
	fprintf(stderr, "process %d, thread %d\n", rank, omp_rank);
